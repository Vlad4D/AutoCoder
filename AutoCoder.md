...
|   |       |   test_repomap.cpp, test_llm_cache.cpp, test_autoverify.cpp,
...
- The system prompt (built once per conversation by `SystemPrompt::build()`)
  embeds a **repo map** from `repomap::build()` (src/agent/RepoMap.h/.cpp):
...
| DeepSeek reserved send tokens | `agent/sendTokens` | `8192` | Headroom kept for the new user message + assistant reply (deducted from `maxContextTokens` before capping). Set via `AgentRunner::setSendTokens()`. |
| Claude reserved send tokens | `claude/sendTokens` | `8192` | Same meaning as DeepSeek reserved send tokens, stored separately per provider. |
...
1. The full conversation state (all messages)
2. The original content of every file that was modified by `write` or `edit`
   during the *previous* turn

### On-disk layout

```
%APPDATA%/AutoCoder/projects/<projectKey>/
+-- <convId>.json                       # Current conversation file
+-- checkpoints/
    +-- <convId>/
...
Each checkpoint JSON file contains `turn_index`, `user_message`,
`messages` (the full conversation snapshot), and `file_snapshots`
